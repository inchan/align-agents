...
test.describe('Dashboard Core - P0', () => {
    ...
    test.describe('Edge Cases (Empty States)', () => {
        ...
        test('E-001: should handle empty stats', async ({ page }) => {
             // Total Syncs should be 0
             // We use precise matching or regex to avoid matching "0" inside "150" if we were using bad mocks, 
             // but here 0 is distinct.
             // StatsCards: <div>{stats.totalSyncs}</div> -> <div>0</div>
             await expect(page.locator(SELECTORS.statsCard).filter({ hasText: /^0$/ })).toBeVisible();
