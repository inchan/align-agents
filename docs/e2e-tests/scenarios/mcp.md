...
| 접근성 | M-028, M-029 | P2 |
| HTTP/SSE 타입 지원 | M-030, M-031, M-032, M-033 | P0, P1 |
...
**목적**: HTTP 타입 MCP 서버를 정상적으로 등록할 수 있는지 확인한다.

**전제조건**: 없음.

**테스트 단계**:
1. **[Add Server]** 버튼을 클릭하여 모달을 연다.
2. Type 드롭다운에서 `http`를 선택한다.
3. 다음 필드를 입력한다:
...
*   요청 body에 `"type": "http"`, `"url"` 필드가 포함되어야 한다.
*   목록에 `e2e-http-server`가 HTTP 타입 뱃지와 함께 표시된다.
...
**목적**: SSE 타입 MCP 서버를 정상적으로 등록할 수 있는지 확인한다.

**전제조건**: 없음.

**테스트 단계**:
1. **[Add Server]** 버튼을 클릭하여 모달을 연다.
2. Type 드롭다운에서 `sse`를 선택한다.
3. 다음 필드를 입력한다:
    *   **Name**: `e2e-sse-server`
...
**기대 결과**:
*   API 호출(POST)이 성공(200/201)한다.
*   요청 body에 `"type": "sse"`, `"url"` 필드가 포함되어야 한다.
*   목록에 `e2e-sse-server`가 SSE 타입 뱃지와 함께 표시된다.
...
**목적**: 각 타입에 맞는 필수 필드가 누락되면 저장이 차단되는지 확인한다.

**전제조건**: 없음.

**테스트 단계**:
1. Type을 `http`로 선택하고 URL 필드를 비워둔다.
2. **[Save]** 버튼을 클릭한다.
3. Type을 `stdio`로 변경하고 Command 필드를 비워둔다.
4. **[Save]** 버튼을 클릭한다.

**기대 결과**:
*   http/sse 타입: "URL은 필수 입력 항목입니다" 에러 표시
*   stdio 타입: "Command는 필수 입력 항목입니다" 에러 표시
*   API 요청이 전송되지 않아야 한다.
...
**목적**: 타입 변경 시 해당 타입에 맞는 입력 필드만 표시되는지 확인한다.

**전제조건**: Add Server 모달이 열려있어야 한다.

**테스트 단계**:
1. Type을 `stdio`로 선택하고 표시되는 필드를 확인한다.
2. Type을 `http`로 변경하고 표시되는 필드를 확인한다.
3. Type을 `sse`로 변경하고 표시되는 필드를 확인한다.

**기대 결과**:
*   stdio: Command, Args, Env 필드 표시
*   http: URL, Headers 필드 표시
*   sse: URL 필드 표시
*   타입 변경 시 이전 타입의 필드 값은 초기화되어야 한다.
...
| 버전 | 날짜 | 변경 내용 |
|------|------|-----------|
| v1.1 | 2025-12-12 | HTTP/SSE 타입 시나리오 추가 (M-030~M-033), 우선순위 조정 |
|      |            | - HTTP/SSE 타입 지원: M-030 ~ M-033 (4개) 추가 |
|      |            | - 기능 매트릭스에 F-08 행 추가 |
|      |            | - M-003 우선순위 P0 → P1 변경 |
|      |            | - M-008 우선순위 P1 → P0 변경 |
|      |            | - 테스트 데이터 master-mcp.json에 type 필드 추가 |
| v1.0 | 2025-12-08 | 초기 작성 - 29개 시나리오 |
