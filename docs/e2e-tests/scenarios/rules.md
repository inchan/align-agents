...
- [엣지 케이스 시나리오](#엣지-케이스-시나리오)
  ...
  - [R-020: 편집 중 다른 Rule 선택 (현재 동작)](#r-020-편집-중-다른-rule-선택-현재-동작)
  - [R-021: 드래그앤드롭 순서 변경 영속성 확인](#r-021-드래그앤드롭-순서-변경-영속성-확인)
- [접근성 시나리오](#접근성-시나리오)
  - [R-022: 키보드 네비게이션](#r-022-키보드-네비게이션)
  - [R-023: 긴 이름 Tooltip 확인](#r-023-긴-이름-tooltip-확인)
- [추가 시나리오](#추가-시나리오)
  - [R-024: Active/Inactive Rule 토글](#r-024-activeinactive-rule-토글)
  - [R-025: 빈 Rule 목록 상태](#r-025-빈-rule-목록-상태)
...
| 드래그앤드롭 정렬 | R-010, R-021 | P1 |
| 에러 처리 | R-011, R-012 | P1 |
| 빈 상태 처리 | R-013, R-025 | P0, P2 |
| 유효성 검증 | R-014, R-015, R-016 | P0 |
| 엣지 케이스 | R-017, R-018, R-019, R-020 | P1, P2 |
| 상태 토글 | R-024 | P1 |
| 접근성 | R-022, R-023 | P2 |
...
// 또는 data-testid 사용
await page.locator('[data-testid="rule-editor"]').fill(content);
```
...
- [ ] 드롭 후 순서 변경
- [ ] 성공 토스트 표시
- [ ] 네트워크 탭에서 reorderRules API 호출 확인
...
### R-020: 편집 중 다른 Rule 선택 (현재 동작)

**우선순위**: P2
**카테고리**: 동작 확인 (Behavior)

**목적**: 편집 중 다른 Rule 선택 시 현재 동작을 확인한다.

**전제조건**: 편집 모드가 활성화된 상태여야 한다.

**테스트 단계**:
1. Rule A를 선택하고 편집 모드로 진입한다.
2. 내용을 수정한다 (저장하지 않음).
3. 목록에서 다른 Rule B를 클릭한다.

**예상 결과**:
- [ ] 편집 모드가 종료되고 Rule B가 선택된다.
- [ ] 수정 중이던 내용은 저장되지 않는다.

**참고**: 미저장 경고 기능은 향후 개선 사항으로 검토 중

---

### R-021: 드래그앤드롭 순서 변경 영속성 확인

**우선순위**: P1
**카테고리**: 영속성 (Persistence)

**목적**: 드래그앤드롭으로 변경한 순서가 새로고침 후에도 유지되는지 확인한다.

**전제조건**: Rule이 3개 이상 존재해야 한다.

**테스트 단계**:
1. Rule 목록에서 드래그앤드롭으로 순서를 변경한다.
2. 변경된 순서를 기록한다.
3. 페이지를 새로고침한다.
4. Rule 목록 순서를 확인한다.

**예상 결과**:
- [ ] 새로고침 후에도 변경된 순서가 유지되어야 한다.
- [ ] 네트워크 탭에서 reorderRules API 호출이 확인되어야 한다.
...
### R-024: Active/Inactive Rule 토글

**우선순위**: P1
**카테고리**: 상태 변경 (Toggle)

**목적**: Rule의 활성화/비활성화 상태를 토글할 수 있는지 확인한다.

**전제조건**: Active 상태인 Rule이 존재해야 한다.

**테스트 단계**:
1. Active Rule의 토글 버튼 또는 메뉴를 클릭한다.
2. 상태 변경을 확인한다.
3. 다시 토글하여 원래 상태로 복원한다.

**예상 결과**:
- [ ] 상태가 Active ↔ Inactive로 전환된다.
- [ ] UI에 상태 변경이 즉시 반영된다 (뱃지/아이콘 색상 변화).
...
**목적**: Rule이 하나도 없을 때 적절한 빈 상태 UI가 표시되는지 확인한다.

**전제조건**: 모든 Rule을 삭제하거나 빈 데이터로 Mock.
...
**예상 결과**:
- [ ] "Rule이 없습니다" 또는 EmptyState 컴포넌트가 표시된다.
- [ ] "Add Rule" 버튼이 표시되어야 한다.
...
|     |            | - 테스트 데이터 Setup 섹션 추가 |
|     |            | - 예상 결과 체크리스트 형식으로 변경 |
| 1.2 | 2025-12-12 | R-020/R-021 실제 구현 반영, R-024/R-025 추가, Monaco 가이드 추가 |
|     |            | - R-020: P0→P2 하향 조정, 현재 동작 설명으로 변경 (미저장 경고 미구현) |
|     |            | - R-021: 영속성 확인으로 변경 (reorderRules API 반영) |
|     |            | - R-010: P2→P1 상향 조정 (드래그앤드롭 완전 작동) |
|     |            | - R-004: Monaco Editor Playwright 구현 가이드 추가 |
|     |            | - R-024: Active/Inactive Rule 토글 시나리오 추가 (P1) |
|     |            | - R-025: 빈 Rule 목록 상태 시나리오 추가 (P2) |
|     |            | - 기능 매트릭스 업데이트 (드래그앤드롭 P1, 상태 토글 추가) |
