...
issue-orchestrator.yaml의 6단계 "프로젝트 자동 결정" 로직을 따릅니다:
...
@.claude/commands/issue-orchestrator.yaml 의 workflow를 따라 서브에이전트를 순차 호출하세요.

코드베이스 탐색 시 Explore 서브에이전트를 활용하세요 (thoroughness: quick).

## 제약사항

@.claude/commands/issue-orchestrator.yaml 의 constraints를 준수하세요.
